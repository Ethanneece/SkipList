...
/**
 * Class skip list using SkipNodes to build it.
 * 
...
public class SkipList<K extends Comparable<K>, V extends Comparable<V>>
{
    ...
    {
        int ranLevel;
        for (ranLevel = 0; Math.abs(ran.nextInt()) % 2 == 0; ranLevel++)
        {
        }
    ...
    /**
     * Removes a SkipNode from the skipList based on key.
     * 
    ...
    public V remove(K key)
    {
        ...
        if (update[0].getForward()[0] == null
                || update[0].getForward()[0].key().compareTo(key) != 0)
        {
            return null;
        }
     ...
     * @return an array list containing values that matched well with the
     *         parameter value.
     */
    ...
    /**
     * Searches for multiple keys by stopping before the first key is found and
     * then keeps going through the SkipList until a key is not found.
     *
     * @param key is the thing being searched for.
     * @return an ArrayList containing all the values of the keys that were
    ...
    public ArrayList<V> search(K key)
    {
        ...
        if (x.getForward()[0] == null
                || x.getForward()[0].key().compareTo(key) != 0)
        {
            return rtn;
        }
    ...
    /**
     * Outputs a print of the SKipList current elements. Uses the Object
     * toString method to represent both the keys and values of the
     */
    public void dump()
    {
        SkipNode<K, V> x = head;
        System.out.println("Node has depth " + x.getForward().length + ", "
                + "Value (null)");
